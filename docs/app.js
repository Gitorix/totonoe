...
const HISTORY_KEY = "totonoe_history"; // ✅ 追加：履歴キーが未定義だとYES/NOで落ちる
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ⚠️ 同名関数が元コード内に2回あるが、ここは元のまま残す（動作優先）
  function showQuestionEditor() {
    app.innerHTML = "";
    ...
    btnSave.addEventListener("click", () => {
      const cleaned = textareas.map((ta) => ta.value.trim()).filter(Boolean);
  ...
  function showFinalQuestion() {
    ...
    document
      ...
      .addEventListener("click", () => finalAnswer(true));
    document
      .getElementById("btnNo")
      .addEventListener("click", () => finalAnswer(false));
  ...
  function finalAnswer(isYes) {
    ...
    // ✅ ここで落ちてるとYES/NOが無反応に見えるので、履歴保存を安定化
  ...
  function deepDiveOne() {
    ...
    document
      .getElementById("btnBackFinal")
      .addEventListener("click", showFinalQuestion);
  ...
  function showResult() {
    ...
    const memo =
      answers.length > questions.length ? answers[answers.length - 1] || "" : "";
